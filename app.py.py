...
def initialize_agent():
    """앱 실행 시 같은 폴더의 모든 PDF 파일을 자동으로 한 번만 업로드합니다."""

    # 절대 경로 직접 지정 (백슬래시 문자로 인한 이스케이프 에러 방지를 위해 r"" 원시 문자열 사용)
    base_dir = Path(r"C:\Users\김문주\Desktop\cosmetic-ai")

    # 지정 폴더가 존재하는지 확인
    if not base_dir.exists():
        raise FileNotFoundError(f"경로를 찾을 수 없습니다: {base_dir}")

    # 같은 폴더에 있는 모든 PDF 파일 탐색
    file_paths = sorted(
    ...
    if not file_paths:
        st.warning(f"⚠️ PDF 파일을 찾지 못했습니다: {base_dir}")

    # 1. 폴더의 모든 PDF를 Gemini 서버로 자동 업로드
    for path in file_paths:
        try:
            file_obj = genai.upload_file(path=str(path))
            uploaded_files.append(file_obj)
        except Exception as e:
            st.error(f"❌ 파일 업로드 실패: {path.name} — {e}")
    ...
    for uploaded_file in uploaded_files:
        ...
        if file.state.name == "ACTIVE":
            ...
            st.error(
                f"❌ 파일 처리 실패: {file.display_name} "
                f"(상태: {file.state.name})"
            )
